...
void Log::LOG(Address *addr, const char * str, ...) {
	...
	if(dbg_opened != 639){
      ...
      //  	fp = fopen("/Users/zou/Desktop/coursera/testcases/dbg.log", "w");
      //  	fp2 = fopen("/Users/zou/Desktop/coursera/testcases/stats.log", "w");
...
void Log::logNodeRemove(Address *thisNode, Address *removedAddr) {
    ...
    LOG(thisNode, stdstring);
//    static char stdstring2[30];
//    sprintf(stdstring2, "%d.%d.%d.%d:%d ", thisNode->addr[0], thisNode->addr[1], thisNode->addr[2], thisNode->addr[3], *(short *)&thisNode->addr[4]);
//    printf("\n %s", stdstring2);
//    printf("[%d] ", par->getcurrtime());
//    printf(stdstring);
//    cout<<stdstring2<<stdstring<<endl;
}
