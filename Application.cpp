...
void Application::mp1Run() {
    ...
    if(par->getcurrtime()==7){
        cout<<"time to check joint"<<endl;
        for( i = par->EN_GPSZ - 1; i >= 0; i-- ) {
            cout<<mp1[i]->getMemberNode()->memberList.size()<<endl;
        }
    }
}
...
void Application::fail() {
	...
	if( par->SINGLE_FAILURE && par->getcurrtime() == 100 ) {
        ...
        int id = 0;
        memcpy(&id, &(mp1[removed]->getMemberNode()->addr.addr)[0], sizeof(int));
        cout<<id<<"Node failed at time"<<endl;
		#endif
